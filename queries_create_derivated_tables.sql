...
CREATE TABLE links_betwenn_gaming_poli_channels_raw2 AS
SELECT yc.author, ARRAY_AGG(distinct ch_id order by ch_id) as channel_ids
          FROM (select video_id, category_e, mp.index as ch_id from (select video_id, category_e, channel_id::channels_200_most_popular as ch_id_enum
                from video_metadata
                where
                ...
                ) as "vm" inner join most_pop_channels_200 mp on mp.enumlabel = vm.ch_id_enum ) as "vm_channels_mapped"
                   INNER JOIN youtube_comments yc on vm_channels_mapped.video_id = yc.video_id
          group by yc.author
          having count(distinct ch_id) >= 2 and count(distinct category_e) >= 2;
